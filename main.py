...
class Patient(BaseModel):
 ...
 name:Annotated[str , Field(...,description="Enter patient's name" )]
 city:Annotated[str , Field(...,description='City where the patient is living')]
 age:Annotated[int , Field(...,gt=0,lt=100,description='Enter patient age')]
 gender:Annotated[Literal['Male' , 'Female' , 'Other'] , Field(...,description="Enter Patient's Gender")]
 height:Annotated[float , Field(...,gt=0,description="Enter Height of the patient in meters")]
 weight:Annotated[float , Field(...,gt=0,description="Enter Weight of the patient in kg")]
...
@app.get('/sort')
def sort_patients(sort_by = Query(...,description='sort on the basis of height,weight or bmi'),order:str = Query('asc' , description='sort in descending or ascending order')):
...
def create_patient(patient:Patient):
 ...
 #check if the patient alreayd exist
 if patient.id in data:
  raise HTTPException(status_code=400 , detail="Patient already exist")

 # Add new patient to the database
 data[patient.id] = patient.model_dump(exclude = {'id'}) # this is dictonary
...
#user can delete patient by only using patient id
